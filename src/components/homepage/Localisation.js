...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      const temperature = parseFloat(data.hourly.temperature_2m[0].toFixed(2));
      setTemp(temperature);
    ...
    <section className="lg:w-[300px] min-w-[300px]">
      <CardContainer containerClassName="">
        <CardBody className="bg-black shadow-lg">
          ...
          {imgSrc ? (<Image src={imgSrc} alt="weather_image" width={250} height={250}/>) : "Loading..."}
          </CardItem>
          <CardItem className="p-4 pt-0 h-36" translateZ={20}>
            <h1 className="text-3xl lg:text-3xl">Localisation</h1>
            <p>City: Paris</p>
            <p className="whitespace-nowrap">Weather status: 
              {isDay === null ? "Loading..." : isDay ? " Day time, " : " Night time, "}
              {weatherStatus ? weatherStatus : "Loading..."} </p>
